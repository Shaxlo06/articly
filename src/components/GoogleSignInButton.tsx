...
export function GoogleSignInButton({ label = "Continue with Google" }: { label?: string }) {
  ...
  return (
    <button
      ...
      onClick={handleClick}
      disabled={loading}
      className="w-full flex items-center justify-center gap-2 text-sm font-semibold px-4 py-2.5 rounded-md border border-border-strong bg-surface hover:border-accent-strong hover:text-accent-strong transition-colors disabled:opacity-60"
    >
